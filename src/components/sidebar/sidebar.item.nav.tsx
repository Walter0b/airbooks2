...
const NavLinks = ({ item, isOpen, handleOpenModal }: Readonly<NavComponentProps>) => (
            ...
            {(isActive) => {
                return (
                    <div className="flex w-full gap-x-2 text-[13px] font-medium leading-6">
                        {item.icon ? (
                            <div className={cn('group-hover:fill-cyan-550', isActive && '!fill-gray-100', 'fill-zinc-550 m-1 w-3 items-center self-center font-semibold leading-6 active:!fill-gray-100')}>
                                <item.icon className={cn(isOpen ? 'w-4' : 'w-full', 'h-full')} />
                            </div>
                        ) : (
                            <CircleIcon className="mr-2 ml-2 w-3 fill-gray-100" />
                        )}
                        <span className={cn(isOpen ? 'hidden' : 'sm:inline')}>{item.label}</span>
                        {item.count && (
...
const MiniNavLink = ({ item, handleOpenModal }: Readonly<NavComponentProps>) => (
    <div className="group/extract z-50 hidden border-y border-r border-gray-300 group-hover:flex sm:min-w-36">
            ...
            {(isActive) => {
                ...
                return (
                    <div className="flex w-full items-center gap-x-2 text-[13px] font-medium leading-6">
                        <span className={cn(isActive ? 'bg-cyan-550' : 'bg-gray-100 group-hover/extract:bg-white', 'absolute -ml-[1.2rem] size-3 rotate-45 border-b border-l border-gray-300 contain-[""]')} />
        ...
        >
            <CrossIcon className="mr-px w-4 rotate-45 fill-gray-100" className1="h-10" className2="w-10" />
        </button>
    </div>
)
    ...
    return (
        <div className="w-full">
            <NavLink
                ...
                className={cn('div2 transition-transform duration-500 ease-in-out', isOpen && 'border-b border-b-gray-200', 'flex w-full flex-row')}
            >
                {(isActive: boolean) => (
                    <>
            ...
            <div className={cn('transition-max-height stop w-full overflow-hidden bg-white text-black duration-500 ease-in-out', isOpen ? 'max-h-[500px]' : 'max-h-0')}>
                {item?.options?.map((subItem, subIndex) => (
                    <NavLink to={subItem.href} key={subIndex} id="navlink">
                        {(isActive) => {
                            return (
                                <div className={cn(isActive && 'bg-slate-100', 'group/option hover:text-cyan-550 relative border-b !border-dotted border-slate-300 p-2 pl-8 before:relative before:flex before:items-center before:pl-4 last:border-none')}>
                                    ...
                                    <div className={cn(isOpen && 'text-can-550', isActive && 'text-cyan-550', 'flex gap-5')}>
                                        <ArrowIcon className={cn('invisible -ml-[1.1rem] w-[0.4rem]', isOpen && ' fill-cyan-550 -rotate-90', isActive && '!visible fill-red-550', 'group-hover/option:visible 0')} />
                                        {subItem.label}
                                    </div>
                                </div>
                            )
...
export const NavigationItem = ({ item, isOpen, handleOpenModal }: Readonly<NavComponentProps>) => (
    <div className="group flex w-full ">
        {item.options?.length ? (
            <Accordion item={item} isOpen={isOpen} />
        ) : (
            <div className="group flex w-full gap-5">
                <NavLinks handleOpenModal={handleOpenModal} item={item} isOpen={isOpen} />
                {isOpen && (
                    <div className="fixed z-10 -mt-px ml-12 bg-slate-100 ">
                        <MiniNavLink item={item} handleOpenModal={handleOpenModal} />
                    </div>
                )}
            </div>
        )}
