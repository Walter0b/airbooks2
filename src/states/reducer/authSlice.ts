...
export const login = createAsyncThunk(
  ...
  async (credentials: { email: string; password: string }, { rejectWithValue }) => {
    ...
    const { access_token: accessToken, refresh_token: refreshToken, expires } = data.data;
...
export const logout = createAsyncThunk('auth/logout', async (reason: string) => {
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(refreshToken.fulfilled, (state, action: PayloadAction<LoginPayload>) => {
        const { accessToken, refreshToken, expiresAt } = action.payload;
        state.accessToken = accessToken;
        state.refreshToken = refreshToken;
        state.expiresAt = expiresAt;
